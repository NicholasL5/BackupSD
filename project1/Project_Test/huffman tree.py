values = {"h":1,"u":1,"f":2,"m":1,"a":1,"n":1}
    ...
    if(stored):
        ...
        if(values):
            ...
            if(candidate_sp[1] <= candidate2[1]):
                new_node = node(candidate1[1] + candidate_sp[2])
...
def dfs(root:node):
    code_dict = {}
    code = ''
    dfs_util(root,code_dict,code)
    print(code_dict)


def dfs_util(root:node,code_dict,code):
    if root is None:
        return
    if(root.left == None) and (root.right == None):
        code_dict[code] = root.val

    dfs_util(root.left,code_dict,(code + '0'))
    dfs_util(root.right,code_dict,(code + '1'))
